...
# Read csv, unpacking the margins as np.array
df = pd.read_csv(filename)

# Calculate the true log count for each margin
for i, row in df.iterrows():
    # Check if the true log count has already been calculated (not nan)
    if row["true_log_count"] is not np.nan:
        # continue
        print(row["true_log_count"])
    print(row["margin"])
