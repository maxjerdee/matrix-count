...
double log_Omega_fixed_diagonal(std::vector<int> ks, int m_in){
    ...
    int m_out = m - m_in;
    double log_Omega = log_binom(m_out + n*(n-1)/2 - 1,n*(n-1)/2 - 1) + log_binom(m_in + n - 1,n - 1) - log_binom(2*m + n*alpha - 1, n*alpha - 1);
    for(int i = 0; i < n; i++){
...
double alpha_constrained(int n, int m, int m_in){
    ...
    double numerator = 2 * (-1 + m) * (-1 + n) * n * (2 + (-1 + n) * n) +
                       mu * (-1 + n) * (4 * m * (2 + (-1 + n) * n) + n * (6 + (-1 + n) * n)) +
                       2 * m * mu * mu * (4 + n * (-2 + n - n * n));

    // Calculate the denominator
    double denominator = n * ((-1 + n) * (-1 + 2 * m + n) * (2 + (-1 + n) * n) +
...
// alpha parameter for zero digaonal sum (case of fixed, but the expression simplifies)
